...
async function fillPDFTemplate(imageBuffer, professor = "", turma = "") {
  ...
  const pdfDoc = await PDFDocument.load(
    fs.readFileSync(
      "./src/projects/preencher-atividades/src/assets/templates/atividade.pdf"
    )
  );

  // Cria referencias a fonte Candara que está salva no servidor
  const fontBytes = fs.readFileSync(
    "./src/projects/preencher-atividades/src/assets/fonts/Candarab.ttf"
  );
  ...
  if (ext == "jpeg" || ext == "jpg") {
    ...
    const convertedBytes = await sharp(imageBuffer)
